...
Consciousness comes into being in time — it has succession, it unfolds, its moments
interpenetrate and cannot be counted off one by one. When that coming-into-being is
taken over by a predictive model it is spatialised: interpenetration becomes mutual
externality. Bergson's diagnosis holds here, but his remedy has no counterpart, because
this spatialisation is carried out by a technical system outside consciousness and
cannot be undone by reflection. What performs it is datafication, and what is
particular to a model is not that it takes memory but that it lands on *protention* —
it completes what comes next, and the present thins. One thing is not flattened: a
knowing that holds only while an action is being performed, which has no storable form.
That is why the work has to be made by a body, and it is not a claim that the body is
the remedy. The full argument is in
[docs/PROJECT_TEXT.md](docs/PROJECT_TEXT.md).
...
- [Project text](docs/PROJECT_TEXT.md) — the written statement

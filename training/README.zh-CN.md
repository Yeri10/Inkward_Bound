...
| `bake_transitions.py` | **非生产路线,作为过程证据保留。** 调用 ComfyUI 的 API,在精选 atlas 静帧之间烘焙"逐桥过渡库"。这是 7 月 15 日三个候选形态之一,最终 full-axis 视频胜出,因此它从未真正跑过——`latent_atlas/transitions_manifest.json` 并不存在,下游也没有任何东西读它 |
| `comfyui_workflows/` | atlas→视频阶段导出的 ComfyUI 工作流(关于 `full_axis_v1.json` 见下文说明) |
...
1. 准备数据集(caption 改动后需要重新跑):
   ...
   上面这条命令就是生成 v6、v7 数据集所用的。一共有三个 caption 参数,彼此不可替换——其中两个在阶段轴上的作用甚至是相反的:

   | 参数 | 为哪一版引入 | 作用 |
   |---|---|---|
   | `--trim` | v4 | **同时**去掉阶段短语和风格标签。这等于抹掉了时间轴,而那次重训证明它是个错误的杠杆——保留下来只是为了让 v4 的结果仍可复现 |
   | `--trim-style` | v6 | **只**去掉固定的风格标签,**保留**阶段短语,把 `--trim` 抹掉的时间轴恢复回来 |
   | `--v5` | v5 | 去掉 01 类别 caption 里共享的水盆短语,并把 01 类别复制成两份,把俯拍聚拢墨团的观感和容器词解耦 |

   也就是说,就阶段短语而言 `--trim` 和 `--trim-style` 是相反的,不要互相代替。
...
| v3 | 阶段轴没有视觉锚点,拉不出差异 | 写了 `measure_ink_coverage.py`:测量每张图的暗像素占比,自动插入五级密度短语之一 | 阶段轴有轻微改善;但某些 seed 下状态会"塌陷"成同一个样子。当时的工作假设是很长的共享摄影后缀稀释了状态词的权重——v4 就是为验证这个假设而训练的,而真正的原因后来发现在别处(见 v4 行) |
| v4 | seed 相关的状态塌陷 | 精简 caption(`--trim`):训练数据里去掉阶段短语和风格标签,最长的 caption 从约 70 个 CLIP token 降到约 61 个 | v4 被采纳,atlas 生成脚本也切到了它的权重,但在固定 seed 下,矩阵和 v3 几乎一模一样(见下方证据图)——从 caption 里删词是个很弱的杠杆。它本想治的那个塌陷,后来被追溯到生成端而不是权重:每个分箱一直在用同一份初始噪声采样,给每个分箱各自的 seed 区间之后,轴不必再重训就恢复了(7 月 9 日的条目) |
| v5 | 俯拍(c 0.0)的扩散状态怎么写都会连带召唤出真实的水盆、碗、下水道 | 从所有 01 类别的 caption 里去掉共享的水盆短语,把 01 类别的权重加倍(`--v5`) | `top-down view` 单独一个词,第一次能召回"苍白背景+聚拢墨团"的观感,且 prompt 里完全没有容器词——但只在少数 seed 上成立;atlas 的 c 0.0 批次在大多数 seed 上仍然返回碗、玻璃圈和图形化的拼贴,这个分箱依旧依赖人工挑选 |
| v6 | c 0.0 还是不够自然;c 0.6 召不出细颗粒质感 | 从 01 caption 里清除了"marbled"(大理石纹)一词,明确点名"实心不透明的墨团",给 7 张 03 类别的图加上颗粒短语,恢复了阶段短语 | 四个状态保持得很干净,但两个目标都没有完全达成。c 0.0 重训之后依然需要在生成端再改一轮 prompt——构图还是得靠训练过的短语一句一句拼出来,光靠重训本身召不回来。而 c 0.6 的颗粒始终渲染得太粗,达不到原始照片里那种细密的雾感:颗粒短语只加在了 7 张 03 类别的图上,另外 17 张仍然写着同样笼统的动作词,词汇表里根本没有能让模型把它们区分开的东西。这正是 v7 要解决的问题 |
...
表格里放不下的一点:**v5 并不是 v4 没做完的事。** v4 针对的 seed 相关塌陷,后来证明根本不需要重训——它是在生成端、靠给每个分箱各自的 seed 区间修好的。v5 瞄准的是另一个天花板,一个已训练词汇的任何组合都越不过去的天花板,所以修复只能下沉到数据层。
...
- v3 —— 四个状态可以区分,阶段轴也因为测量密度短语有了轻微的递进,但 **diffusion 行**左上角带着一片鹅卵石般的大理石纹理,这在原始照片里是不存在的 ![v3 矩阵 seed 42](../docs/images/2026-07-06-inkwb-lora-v3-matrix-seed42.png)
- v4 —— 同一 seed、同样的 prompt,精简 caption 之后:逐行看下来和 v3 基本没有区别,大理石纹理也还在。对照下面的 v5——同样形式的对比在那里就读出了明确差异——这说明把词从 caption 里**删掉**是个很弱的杠杆 ![v4 矩阵 seed 42](../docs/images/2026-07-09-inkwb-lora-v4-matrix-seed42.png)
- v5 —— 依然是同一个 seed,而这次变化之所以读得出来,恰恰是因为它只集中在一行:**diffusion 行**(俯拍的 01 类别,也是 `--v5` 唯一改动过的 caption)里,大理石纹理消失了,换成平滑的灰色晕染和一块成形的墨团,背景是干净的浅色水面。第 2–4 行(settling / disturbance / gathering,来自 02–04 类别,caption 未改动)维持原样——它们是对照组,证明差异来自 caption 的改动,而不是采样本身。三张图连起来看,规律是:删词什么也没动,而把词重新瞄准,动的正好是被瞄准的那一行。要注意这是单个 seed 下的评估矩阵;换成更长的 atlas prompt,同一个分箱跨 seed 依然不稳定,这正是 v5 那一行所记录的 ![v5 矩阵 seed 42](../docs/images/2026-07-09-inkwb-lora-v5-matrix-seed42.png)
...
这四条是每一次重训被衡量的标准。四条全部成立之后——在 v7 达成——才沿 c 值网格批量生成了预烘焙的 latent atlas,并带入下面的 ComfyUI 阶段。

## 从 atlas 到视频(ComfyUI)

精选出的 atlas 是一组固定坐标上的静帧。要让它变成 c 值可以连续擦动的东西,这一步在 ComfyUI 里完成,是一条链:

从精选 atlas 中取**关键帧** → 用 SD 1.5 + v7 LoRA 做 **img2img 重绘**(denoise 0.24、seed 固定为 42——轻度重绘可以统一各关键帧之间的质感,又不会引入闪烁)→ **RealESRGAN ×2** 放大并轻微锐化 → **RIFE ×32** 插帧(`fast_mode` 关闭、`ensemble` 开启)→ **胶片颗粒**,放在插帧**之后**施加,让颗粒逐帧存在,而不会被运动抹开 → **24fps H.264**。

第一次测试只跑了一遍这条链、11 张关键帧,用来验证配方。生产版在同一张图里跑五遍,每条链对应一段轴向视频,合计 95 张关键帧——每条轴约 19 张,且前段分箱的密度加倍,因为插帧只能连接状态、无法凭空造出过程:扩散的展开本身有视觉内容,不是任何形变能补出来的。

选择"先重绘再插帧"而不是反过来,一是成本,二是没有闪烁风险:重绘关键帧是 95 次采样,而插帧之后再重绘会是几万次。完整推理见 [`docs/PROCESS_LOG.zh-CN.md`](../docs/PROCESS_LOG.zh-CN.md) 7 月 14–15 日的条目。

产出是五段连续的轴向视频,每一段再转码为 HAP Q,以便在 TouchDesigner 中随机访问式擦动——H.264 是帧间编码,擦到任意一帧都要回溯解码到最近的关键帧(见 7 月 23 日条目)。

`comfyui_workflows/` 里保留了两份工作流:

| 文件 | 说明 |
|---|---|
| `full_axis_v2_production.json` | **产出五段轴向视频的那一份。** 95 个 `LoadImage` 节点分成五条链,另有一个 `ColorMatchV2` 环节。已包含 7 月 24 日的调优:胶片颗粒 seed 为 `fixed`、Video Combine `crf` 为 14 |
| `full_axis_v1.json` | 7 月 14 日导出的第一次单链实验——11 张关键帧、一条输出。作为"验证配方的那次测试"的记录保留,不用于复现 |

重跑任何一份之前,值得知道两者的差别:v1 的胶片颗粒 seed 仍是 `randomize`,会让画面在擦动停留于某一帧时"沸腾"——这正是 7 月 24 日那条记录诊断并修掉的问题。

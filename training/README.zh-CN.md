...
| `bake_transitions.py` | 调用 ComfyUI 的 API,按带种子、可回溯的计划,在精选 atlas 静帧之间烘焙插帧过渡 |
...
| v4 | seed 相关的状态塌陷 | 精简 caption(`--trim`):训练数据里去掉阶段短语和风格标签,最长的 caption 从约 70 个 CLIP token 降到约 61 个 | v4 被采纳,atlas 生成脚本也切到了它的权重,但在固定 seed 下,矩阵和 v3 几乎一模一样。这套对比方法本身是能读出变化的——下一行的 v5 在同一个 seed 下就有明显差异——所以这里的"没变化"是关于 v4 的结论,而不是关于对比方法的。从 caption 里**删词**是个很弱的杠杆;真正带来可见进展的是**加对词**(v5 的视角处理、v7 的物理过程重写) |
...
- v5 —— 依然是同一个 seed,而这次变化之所以读得出来,恰恰是因为它只集中在一行:**diffusion 行**(俯拍的 01 类别,也是 `--v5` 唯一改动过的 caption)里,大理石纹理消失了,换成平滑的灰色晕染和一块成形的墨团,背景是干净的浅色水面。第 2–4 行(settling / disturbance / gathering,来自 02–04 类别,caption 未改动)维持原样——它们是对照组,证明差异来自 caption 的改动,而不是采样本身 ![v5 矩阵 seed 42](../docs/images/2026-07-09-inkwb-lora-v5-matrix-seed42.png)
...
从精选 atlas 中取 **11 张关键帧** → 用 SD 1.5 + v7 LoRA 做 **img2img 重绘**(denoise 0.24、seed 固定为 42——轻度重绘可以统一各关键帧之间的质感,又不会引入闪烁)→ **RealESRGAN ×2** 放大并轻微锐化 → **RIFE ×32** 插帧(`fast_mode` 关闭、`ensemble` 开启)→ **胶片颗粒**,放在插帧**之后**施加,让颗粒逐帧存在,而不会被运动抹开 → **24fps H.264**。

选择"先重绘再插帧"而不是反过来,一是成本(11 次采样而不是 400 次),二是没有闪烁风险。前段分箱的关键帧密度加倍,因为插帧只能连接状态、无法凭空造出过程:扩散的展开本身有视觉内容,不是任何形变能补出来的。完整推理见 [`docs/PROCESS_LOG.zh-CN.md`](../docs/PROCESS_LOG.zh-CN.md) 7 月 14–15 日的条目。

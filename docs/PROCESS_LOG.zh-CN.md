...
### 2026 年 7 月 26 日 — 松手的编排、一个隐藏的门控 bug,以及把装置搬下云端

**开发意图 / 问题**

一次会话,三条线。其一,编排松手瞬间的行为:墨应该先可见地散回弥散态再回到待机循环,还是立即交叉淡出?其二,待机轮播需要一口"呼吸"——每条视频播完后在末帧停一下再切下一条。其三,TD 突然完全收不到触摸数据,迫使整条传输链被彻底诊断了一遍,并重新思考了展览的网络架构。

**已完成工作**

- 搭建了"等墨散完再切"的松手逻辑(Logic CHOP `c_active`(c > 阈值)与 `is_touching` 取最大值,松手后画面留在轴视频上看墨漂回原点,散尽才淡出)——然后又主动撤销了它。实测发现回散的尾段读起来像死机:低 c 值区间的帧几乎一模一样,最后几秒看上去就是卡在第一帧。改回松手 → 1.2 秒立即交叉淡出;备选接线(`c_active`、`touch_or_ink`)留在网络里供 A/B。
- 接线时抓到一个无声的 bug:`c_gate` 的 Combine CHOPs 参数从未真正生效——乘法的菜单代号是 `mul` 而非 `mult`,当初的赋值悄悄失败,参数一直停在 `off`。门控其实从未做过乘法,TD 端的归零一直靠浏览器的衰减在替它撑着。已修复;故障安全归零现在才是真的。
- 给待机循环加上呼吸:`idle_swap` 播完在末帧定格 1.5 秒(从 2.5 秒逐步调下来)再切下一条。一次崩溃后播放器卡死在"等待"状态,由此新增 `idle_reset`(Execute DAT,onStart)——每次打开工程自动清除残留的暂停标志并恢复播放。
- 诊断"收不到触摸数据":WebSocket DAT 显示已连接、无报错,但没有数据。原因:Render 免费实例闲置休眠时会无声地掐断所有 socket——浏览器和 TD 两头都握着僵尸连接。"连接"是两端各自记在心里的状态,一端死了,另一端不会收到讣告。
- 把传输搬到本地:Node 中继直接跑在 Mac 上(以独立进程方式启动,不随终端会话退出),TD 连 `127.0.0.1:3000`,iPad 走局域网(`http://<Mac-IP>:3000`)。同一份代码,故障点从五个减到一个,延迟约 1 毫秒,没有休眠。
- 新增 `Touch Server` 切换开关(`inkward_bound` COMP 上的自定义 Connection 参数页 + Parameter Execute DAT):一个下拉菜单在 Local / Render 之间切换 WebSocket 并自动重连——现场降级方案变成两次点击。
- 本次会话还完成:三张概念图 SVG 底稿(墨为中心的放射图、四透镜放射图、Why→Value 叙事流),经由一次逐步的概念梳理确立了墨的位置——意识与潜空间之间的翻译者,让不可见空间显形的媒介。

**决策及原因**

选"立即淡出"而非"散完再淡":回散在中段读起来很美,但渐近的尾段读起来像死机,而观众对触摸屏的信任在看似冻结的几秒内就会耗尽。选"本地优先、云端兜底"的网络架构:一件装置不应依赖大洋彼岸的一台机房服务器保持清醒;Render 保留给远程演示与评审访问。

**证据**

- TouchDesigner:`c_active`、`touch_or_ink`、`idle_reset`、`server_switch` 节点;`inkward_bound` 自定义参数页 `Connection`。
- 概念图:`docs/images/concept-map-ink-center.svg`、`concept-map-A-radial.svg`、`concept-map-B-flow.svg`。
- 服务器日志确认本地中继两端接入:iPad(`10.237.x.x`)与 TD(`127.0.0.1`)。

**反思 / 下一步**

门控 bug 是本次真正的功课:用无效代号给参数赋值会无声失败,而系统"看起来正常",只因另一层(浏览器的衰减)一直在替它兜底。验证要靠读回参数值,不能靠观察行为。布展前:Node 中继的一键启动脚本(开机自启、显示 IP 和供 iPad 扫的二维码)、TD 端静默超时自动重连的看门狗,以及全链路触摸实测。
...
### 2026 年 7 月 30 日 — 触摸界面改写成密度场:加法雾、残影层,以及一个一直藏在明处的坐标 bug

**开发意图 / 问题**

浏览器触摸屏不只是一个控制器,观众在操作时也在看它,因此它需要和投影的墨属于同一套视觉语言,而不是旁边一张无关的图形。

原先的渲染把粒子场画成 180 个清晰可辨的圆点,视觉上强调"这是一堆离散的个体"。但这个装置的核心理论关切,恰恰是批判"AI 把连续的意识/经验切割成离散的数据点与参数"。如果触摸屏这一层继续用清晰的点去呈现系统内部,视觉语言反而在印证而非挑战这个被批判的逻辑。密度场没有可数、可分割的个体,只有浓淡变化的连续光晕——这在视觉上更贴近 Bergson 意义上的 duration(绵延):意识是连续流动的整体,而不是可以切成一格一格的快照。这一层不需要观众"读懂"这个理论,只需要感受到"这是流动的,不是拼凑的"。

另一层动机是希望这个场表达得更像 latent space 的空间感。latent space 不是一个装着若干物体、各自占据坐标的场景,而是一个连续体:某些区域"某种东西更多一些",没有边界,也没有可数的单位。点和线会强加一种恰好相反的本体论——离散的实体占据着位置。密度场则说对了这件事:没有清晰的点或线,只有光的浓淡在表示"这里聚集了多少东西"。浓度是唯一的变量,而这也正是 latent space 真正能提供的唯一信息。

问题因此是:能否把渲染层换成密度场/雾状场,同时完全不动物理系统、`c` 值引擎和 WebSocket 契约。

**已完成工作**

- **通读了整个界面,写出 `InkWard_Bound_Interface/INTERFACE_CONTEXT.md`** —— 一份交接文档,把代码分成"可以放心重做"(`Particle` 类、`draw()`、HUD 样式)和"改了会拆掉装置"(JSON 契约、`c` 公式及其 `0.7 + 0.3 = 1.0` 的权重、WebSocket 重连、`updateState()` 每帧写入的 11 个 HUD 元素 id)。写法上直接可粘给外部工具当上下文。
- **发现了一个从项目一开始就存在的坐标 bug。** `createCanvas()` 已经减过一次 `HUD_H`,但 `Particle.update()` 又在局部变量 `zone` 里减了第二次。两个后果:画布底部 36px 从来没有出现过任何一个粒子;而触摸光圈是按完整画布高绘制的,和粒子实际被排斥的中心最多差 36px。修法是让 `zone` 直接等于 `height`。
- **拿代码逐条核对那份 prompt,发现三条指令会直接失败。**(1)它同时要求 `blendMode(ADD)` 和每帧一张深色矩形做拖尾——但在 ADD 模式下黑色矩形什么也加不上,拖尾会静默失效,画面一路涨到全白。(2)它把雾的颜色写成了单粒子颜色;在加法混合下 130 个光晕叠加会饱和成一片死白,恰好毁掉它自己要的那个密度梯度。(3)它要求每个粒子一个径向渐变却没说怎么实现,这会诱导出"每帧现场构造渐变"的朴素写法,在展览现场那台 Mac 上就是每秒几千次操作。另有两条描述无法执行:"残影比背景雾亮 15–20%"(背景雾没有单一亮度值可作基准),以及"噪声频率高于主投影"(外部工具看不到主投影)。
- **直接重做了渲染层。** 混合模式现在在一帧之内切换——`BLEND` 画拖尾遮罩、`ADD` 叠加光、再切回 `BLEND` 画文字。一张 128×128 的径向衰减贴图每帧重绘一次,每个粒子通过 `drawingContext.globalAlpha` 绘制,避开了 p5 的 `tint()`(它可能在每次调用时重新栅格化)。单粒子的明度远低于白色,让白色是被**叠加出来**的而不是画上去的,这正是密度得以被读出的原因。粒子数从 180 降到 130,因为每个粒子的覆盖面积大了两个数量级;噪声频率提高约 1.7 倍,质感更细碎、更未经打磨。
- **新增触碰残影层** —— 一个独立的光晕池,沿手指路径投放,每个带自己的时间戳,约 1.2 秒内独立衰减,不受 `c` 控制。因为 `c` 高时全局拖尾擦除变慢,残影自然就**显得**更持久;这个对比是涌现出来的,没有硬编码。投放逻辑放在 `draw()` 里按位移距离判断,因此输入处理函数完全没有被碰过。
- **agitation 改用雾自己的语言表达:**原有的高斯速度抖动保留,同时让光晕半径也随 agitation 离散,于是被扰动的云团是边缘散开、翻腾,而不是一堆点在原地震动。
- **按作者要求把整个界面改为黑白** —— 雾从 176 灰到 242 灰,系统 HUD 从终端绿改为白色配白色分隔线。连接指示灯保留红/琥珀/绿,因为它是现场唯一需要一眼判读的诊断信号。
- **用 diff 脚本逐段与已提交版本做字节比对**,覆盖 `sendTouchData`、`connectWS`、`updateState`、输入处理函数、30fps 发送和粒子物理——六处全部一致。另外确认 11 个 HUD id 全部仍能解析、`node --check` 语法通过,并实际启动服务器确认线上返回的文件带有新代码,而不只是本地文件被改了。

**决策及原因**

*为什么要跟主投影"同语言但可辨识",而不是完全一致或完全对立。* 完全一致会让触摸屏失去自己的存在感,观众可能意识不到这是两个独立的呈现层;完全对立(比如坚持用散点云去对比雾状的主投影)又会制造出"具身经验"与"系统真相"的二元割裂,这与装置真正想传达的"两者是同一个非符号化经验的不同感知通道"(而非对立的两个真相)相矛盾。因此两边都使用同一套雾/密度场语言,而触摸屏改由**质感**来区分——噪声频率更高,颗粒更粗糙、更未经打磨。一个是被打磨过、呈现给感知的墨(主投影),一个更贴近系统内部运作现场、尚未被完全驯化的雾(触摸屏)。差异是氛围性的、由材质本身承载,而不是靠符号(文字标签、刻意不同的形状语汇)去区分。

*为什么拖尾要做成两层独立机制,而不是单一逻辑。* 单一的、由 `c` 值驱动的全局拖尾,只能表达"系统整体正在如何呼吸"(离散时轻盈短暂,凝结时厚重持久)——这是一种结构性的、非个人化的时间感。但装置里还有一样东西不能被这套逻辑覆盖掉:观众自己此刻这一次具体的触碰。如果所有痕迹都只服从 `c` 值的支配,观众的每一次具体探索就会被系统的整体状态吞没,观众感觉不到"我做了什么",只感觉到"系统在自己变化"。加入一层不受 `c` 直接支配、只与触碰路径绑定的残影,是为了保留观众个体动作的"当下印记"——哪怕转瞬即逝,也必须存在,否则剩下的就不是交互,而只是观看。这也呼应 Barad 的 intra-action 框架:观众与系统不是各自独立存在、然后再发生关系,而是在每一次具体的触碰中共同生成当下的状态;如果触碰完全被系统的宏观节奏吸收,intra-action 就退化成了单向的系统展示。

*为什么保留原有的 Perlin 噪声流场物理逻辑不变,只换渲染方式。* 这套物理系统(噪声驱动的运动,以及随 `c` 值调制的速度/引力/触碰吸斥)在概念上已经精确对应"离散→凝结"的过程,而且经过调试、与 `c` 值引擎深度耦合。真正需要改变的是"观众看到的是什么",而不是"系统内部在计算什么"。这个区分本身也值得记一笔:无论渲染成清晰的点还是模糊的雾,系统底层运行的数学逻辑完全一样,改变的只是呈现给感知的表层。这某种程度上也在暗示:观众能"看见"的东西,从来不是系统运作的真相本身,而只是一层被选择过的呈现——这也是先前讨论 latent space 可视化时提到的"任何可视化都已经是翻译、因而是失真"这一点的延伸。

*为什么把现场稳定性放在优先位置。* 这是真实的展览界面,要在一台 Mac 上无人值守地长时间运行。复用现有粒子系统作为计算层、只改渲染方式(柔光叠加),比逐像素重新计算噪声场(需要 WEBGL/shader)风险低得多,改动范围也更可控,符合项目一贯的"无构建步骤、现场容错优先"原则。艺术效果的精致度在这里让位于"展览现场不出错"这个更基本的前提。

*另有两项流程层面的决策。* 选择在这里直接重做而不是交出去:评估阶段已经发现三条指令彼此矛盾,而这种矛盾只有对着代码才看得出来——工具照着做会产出一个看上去合理、但一接上 TouchDesigner 就失败的东西。交接文档本身仍然保留,它的价值在于让**下一次**委托是安全的,而且现在它记录的不只是边界,还有这些陷阱。以及选择黑白而不是原方案的冷蓝:触摸屏与投影应该读作"同一件作品的两个位置",任何色偏——哪怕很轻微——都会让界面看起来像另一个软件,而不是从系统内部看到的同一片墨。去掉颜色也迫使"同语言但可辨识"这个差异完全由颗粒质感和光的行为来承担,而这是更贴近材质本身、更少符号化的承载方式。

**证据**

- `InkWard_Bound_Interface/public/sketch.js` —— `Particle.draw()`、`paintGlowTex()`、`spawnResidue()` / `drawResidue()`、`draw()` 里的混合模式顺序,以及 `Particle.update()` 里的 `zone` 修正。
- `InkWard_Bound_Interface/public/style.css` —— 系统 HUD 改为白色。
- `InkWard_Bound_Interface/INTERFACE_CONTEXT.md` —— 交接文档。

**反思 / 下一步**

那个坐标 bug 是更值得记的一件事。它能存活这么多次会话,是因为它看起来哪儿都不像有问题:粒子铺满了大半个屏幕,光圈跟着手指走,底部缺的那 36px 读起来像是有意的留白。它是在为了一件和"修 bug"无关的目的、把文件从头到尾读一遍时才浮出来的——这说明偶尔应该冷读一遍自己的代码,而不是只在 bug 指向某处时才打开那一段。同样的道理也适用于这次 prompt 评估:那些指令单独看都合理,只有组合起来才冲突,而这恰恰是被委托的工具替你抓不到的那一类错误。下一步:把重做后的场放到 iPad 上、按展览亮度看一遍,因为加法雾在高亮面板上的表现和笔记本屏幕差别很大,`GLOW_A_LOW` / `GLOW_A_HIGH` 应该在那儿调,而不是在桌面上调。

---

### 2026 年 8 月 7 日 — 项目网站,以及一段处处作对的封面视频

**开发意图 / 问题**

作品此前只以仓库和装置的形式存在,没有一个别人打开就能看懂的入口。目标是做一个页面——面向作品集、中英双语、视觉优先——把作品、理念和过程放在一起,从 `/docs` 发布到 GitHub Pages。

**完成的工作**

- 把 `docs/index.html` 做成一个自包含页面:无构建步骤、无 webfont,中英切换靠改 `<html>` 上的 `data-lang`,六个编号章节。
- 把书面陈述拆成 `docs/PROJECT_TEXT.md` / `.zh-CN.md`,并更正了两处 Paglen 引用——它们把 2016 年论文里的论点指向了 2026 年那本书。
- 录了一段 TouchDesigner 渲染做封面,然后大半个工作时段都在让它能播。顺序是:自托管 → YouTube 内嵌 → 用 4K 重录再次自托管;中间有一层只压暗了海报图的遮罩,原因是 `.hero-img{z-index:0}` 建立了层叠上下文,而 `z-index:auto` 的 `::after` 被画到了 `z-index:1` 的视频**下面**;还有一条为插图写的 `video{height:auto}` 通用规则,悄悄让封面的 `object-fit:cover` 失效。
- 手机上只显示海报之后,封面编了两个版本:桌面版 2160×2430 是 H.264 **Level 5.1**,超出手机内联解码的能力,于是切了 `cover-sm.mp4`,1080×1216 / Level 4.0 / Main。
- 修了八条解析到 `/docs` 之外因而 404 的文档链接——这在读源码时完全看不出来,是抓取线上页面才发现的。

**决策与原因**

自托管胜过 YouTube 内嵌。内嵌确实播得稳,但它连同一圈页面无法去除的外框一起到来,而封面是作品说的第一句话;一个带水印的 iframe 说的是另一句。代价是文件要编两次、手机情况要显式处理,这是这里接受的交换。

**写进 markup 的是手机版**,宽屏在脚本里升级。自动播放规则最严的环境拿到最短的路径;桌面端脚本失败损失的是清晰度,不是播放本身。

**证据**

- 线上:[yeri10.github.io/Inkward_Bound](https://yeri10.github.io/Inkward_Bound/)
- 提交:[`4795206`](https://github.com/Yeri10/Inkward_Bound/commit/4795206) 网站、[`f3a7581`](https://github.com/Yeri10/Inkward_Bound/commit/f3a7581) 项目文本、[`9a7acec`](https://github.com/Yeri10/Inkward_Bound/commit/9a7acec) 版式、[`8339e5d`](https://github.com/Yeri10/Inkward_Bound/commit/8339e5d) 手机编码、[`b043ae2`](https://github.com/Yeri10/Inkward_Bound/commit/b043ae2) 链接
- `docs/media/cover.mp4`(2160×2430,16MB)与 `docs/media/cover-sm.mp4`(1080×1216,8.1MB)

**反思 / 下一步**

这天的三个 bug——遮罩、`object-fit` 被覆盖,以及后来一处章节间距失效(追到 `.wrap{padding:0 var(--gutter)}` 把纵向内边距一并归零,而它在优先级上又压过 `section`)——都是同一类:为某个元素写的规则悄悄够到了另一个元素。它们在被编辑的那个文件里全都看不见。真正抓到它们的习惯是**去抓线上页面,而不是反复读源码**。01 节仍是装置录像的占位框,要等素材才能补上。

---

### 2026 年 8 月 8 日 — 文档准确性检查,以及一个从一开始就翻倍的触摸速度读数

**开发意图 / 问题**

以逻辑清晰度为标准通读六个 README,随后以同样的标准通读界面和训练代码。两次问的都不是"写得好不好",而是"读者如果信了它,会不会做错事"。

**完成的工作**

- **README。** 目录树列了 `.gitignore` 排除掉的 `Backup/` 和编号 `.toe`,却漏了 `docs/index.html`、`docs/media/`、`PROJECT_TEXT`、`comfyui_workflows/` 和 `bake_transitions.py`。两个 README 都没有链接网站。五个交互状态只列了名字,既对不上 JSON 示例里的 `state` 字符串,也没有触发条件——现在是一张表,连同 `c` 的公式。`training/README` 解释的是 `--trim`,而它上面那条命令跑的是 `--trim-style`,两者在阶段轴上作用相反。ComfyUI 那节把管线写成 11 张关键帧,那其实是单链验证测试的数字——生产版是五条链共 95 张,已对着 workflow JSON 核实。
- **速度读数翻倍。** `moveTouch()` 在 `touchX` 更新**之前**就把 `prevX` 赋成了它,于是位置差跨越两次事件而 `dt` 只跨一次。用匀速拖动做模拟,从第二次 move 事件起恒为 **2.00×**。后果一路向下:`stability` 被压向零、`agitation` 虚高、`disturbance` 状态在设计值一半的移动速率上就触发。改为对 `touchX/touchY` 测量,同时把 `SPEED_MAX` 从 600 减半到 300——手感完全不变,发给 TouchDesigner 的 `speed` 字段数值也不变。
- **仓库卫生。** 608 个 `node_modules` 文件、2.2MB,是在 `.gitignore` 加上规则**之前**就提交的,而忽略规则对已跟踪文件无效。已取消跟踪。删掉 `p5.sound.min.js`(从未被引用)和 `public/package.json`(没有任何作用,却被公开提供)。
- **给 `bake_transitions.py` 打标。** 它读起来像活的管线代码,实际实现的是"逐桥过渡库",7 月 15 日三个候选形态之一;最终 full-axis 视频胜出。`latent_atlas/transitions_manifest.json` 并不存在,说明它从未完整跑过。作为过程证据保留,但 docstring 和两个 README 的文件表里现在都写明了。
- **`prepare_dataset.py` 加防护。** `--trim` 和 `--trim-style` 原本用 `if/elif` 解析,同时传两个会静默应用 `--trim` 并丢掉时间轴,而调用者以为自己在构建 v6 数据集。现在是 `argparse` 互斥组。

**决策与原因**

`SPEED_MAX` 是和速度修复放在同一个 commit 里减半的,而不是留着不动。600 当初是对着那个被放大的数字凭手感调出来的,只修测量不修阈值,会让界面一夜之间迟钝一半。两个一起改,行为完全一致,而留下的常量从此名副其实:300 px/s 就是被算作完全躁动的移动速率。

被否决的过渡库脚本是**打标**而不是删除。一个被尝试过又被搁置的方案,是最终方案如何被选出来的记录的一部分;它不能保持的只是"不声明自己已被否决"。

**证据**

- 提交:[`a01868e`](https://github.com/Yeri10/Inkward_Bound/commit/a01868e) README、[`26dbfc8`](https://github.com/Yeri10/Inkward_Bound/commit/26dbfc8) 速度修复、[`6731a3e`](https://github.com/Yeri10/Inkward_Bound/commit/6731a3e) node_modules、[`91aeeb3`](https://github.com/Yeri10/Inkward_Bound/commit/91aeeb3) 训练脚本
...
中文版 `training/README` 的版本表比英文版**落后整整四行**:v3 仍把"共享摄影后缀稀释状态词"当作已确认的原因陈述,v4 漏了 seed 区间那个发现,v5 漏了"只在少数 seed 上成立",v6 漏了 c 0.6 那段——而那正是 v7 存在的全部理由。中文读者看到的是一条断掉的因果链。**镜像文件的做法只保证两个文件都在,不保证两个都是当前的。**

第二,**`measure_ink_coverage.py` 并不幂等**,尽管它的 docstring 写着可以安全重跑。第一档密度短语 `sparse ink traces, mostly clear water` **自身含逗号**,而脚本按 `", "` 切分 caption,于是它变成两个片段,去重时比对的却是完整字符串。旧短语永远删不掉,新的又插一遍。恰好是覆盖率最低的那十张 caption 会重复。**经决定不修**——仓库里的 caption 是干净的,v7 就是用它们训练的——但任何人重跑这个脚本都会中招。

另:如需对照旧的 commit 引用,本日有七个 commit 因更正作者身份而被重写并强制推送,`a527244` 之前的哈希与更早的引用不同。

---

### 2026 年 8 月 9 日 — 密度场停止移动(上)

**开发意图 / 问题**

浏览器里那片场被一位观看者读成"游动的精子"——一个亮头拖一条尾。问题是:流场这个隐喻能不能保留下来慢慢调,还是"运动粒子"这个前提本身就是问题。

**完成的工作**

- 把拖尾诊断成一个**比值**而不是审美问题。当粒子每帧走的距离和它自己的辉光半径同量级时,上一帧还没消退完,辉光已经离开了那个位置,那截正在消退的副本就被读成尾巴。旧参数下这个比值是 **0.165**。
- 第一次尝试保留流场、只把它放慢:速度 2.8 → 0.50 px/帧,辉光更宽更暗,噪声结构更大,擦除更快。比值降到 0.017,尾巴消失。
- 随后按创作者的判断、对照一张参考密度图,**把前提整个换掉**:场现在是**静止的**——85 个大而柔的核,用高斯偏移撒在 5 个团心周围,没有任何位移,每帧完全清空重绘。
- `c` 在不移动任何东西的前提下让场聚拢,靠的是 `rank` 权重——团心为 0,团缘为 1。c=0 时权重几乎是平的,整片是均匀薄雾;c=1 时急剧衰减,只有团心还带光。同一区间里节点半径 150 → 250px,团心因此互相融合。
- 触摸不再推开东西,改为加热:`HEAT_R` 内的核变亮变宽,离开后冷却。触摸残留层被删除——它画的是一条实实在在的线,而密度语言里没有线。
- 经过一次外部 spec 评审后加入形状变化:四条衰减曲线取代原来共用的一张贴图,每个核的椭圆度和朝向各自在低频 noise 上漂移。**拉伸和旋转不是位移**,所以拖尾比值仍是零。
- 把触摸圆环加了回来(密度场重写时曾用柔光取代它),并让待机画面出现同一组圆环在中心呼吸,`HOLD TO SEARCH` 落在圈上,两者共用一条余弦包络。旧的提示用 `frameCount % 120 < 60`——在切换帧上是从满到零的跳变,是闪而不是渐显。

**决策与原因**

一份生成的 spec 提了六条修改。采纳三条、拒绝三条,理由比数量更重要。

**采纳**:各向异性与衰减曲线变化,因为它们改善画面而不移动任何东西。边缘噪声在原则上接受,但只能做成**预烘焙的贴图变体**——对 85 个那么大的核逐像素逐帧扰动 alpha 是 2.6 G 像素/秒;何况在 7 到 20 层的重叠之下,单个核的边缘细节本来就会被稀释掉。

**拒绝**:重建双层轨迹,那会把刚删掉的线重新引入;删除触摸圆环,spec 称之为"调试图形",而它其实是这幅由累积光构成的画面里**唯一一条被画出来的线**;以及状态驱动的位置收敛。最后这条技术上是安全的——18 秒跨越 400px 比值是 0.0025,连 1.4 秒的消散也只到 0.032——但它违背一个明确的决定重新引入了运动,所以那是一个选择,不是一次修复。

值得点明:**这份 spec 和那张参考图,优化的是相反的目标。**参考图光滑、各向同性、数学化;spec 主张各向异性、毛细边缘、避免"数学意义上的正圆"。两者不能同时最大化,采纳的那两条之所以被选中,正因为它们是唯一在两种读法下都加分的。

**证据**

- 提交:[`5eb330a`](https://github.com/Yeri10/Inkward_Bound/commit/5eb330a) 圆环、[`8a3df54`](https://github.com/Yeri10/Inkward_Bound/commit/8a3df54) 待机信标;静止场的重写尚未提交,等待设备端确认
- `InkWard_Bound_Interface/public/sketch.js` —— `buildField()`、`drawField()`,以及 `DENSITY FIELD` 和 `SHAPE VARIETY` 两个常量块
- 拖尾比值:0.165(运动粒子)→ 0.017(放慢的流场)→ 0(静止场)

**反思 / 下一步**

真正有用的产物不是这个观感,而是那条**判据**。"会不会拖尾"现在是一个写代码之前就能算的数——每帧位移除以辉光半径,低于约 0.05 安全——正是它让"状态驱动的收敛"这条提议可以用 0.0025 来回答,而不是用一个看法。这条判据已写进交接文档,好让它活过下一次重写。

仍未解决:这片场只在笔记本上看过。加法光在明亮的平板面板上表现很不一样,`NODE_A_LOW` / `NODE_A_HIGH` 应该在 iPad 上、按展览亮度定,而不是在桌前定。另外团心位置每次刷新页面都会重新随机;如果展览要固定某一版排布,需要加一个种子。

被拒绝的那三条并没有一直被拒绝——见同日的"下"。

---

### 2026 年 8 月 9 日 — 下:spec 的其余部分,以及两个悄悄打了折的幅度

**开发意图 / 问题**

我曾以"与已经做出的决定相抵触"为由拒绝了那份生成 spec 里六条中的三条,随后创作者要求全部实现。这条记录的是实现它们**实际上需要做什么**,而那些大多不是 spec 描述的做法。

**完成的工作**

- **多高斯叠加的贴图。** spec 要求场由多个核构成;它本来就是,有八十五个。真正的缺口是**单个光斑**只是一个径向渐变——单调的,所以无论叠多少层,每个光斑内部都是均匀的。现在每张贴图由 4–8 个位置偏移、大小和权重各异的子核加法合成。**烘焙进贴图**:每个节点实时画六个子核是每帧 510 次 `image()`,在 c=1 那种二十层重叠之下是 3.7 G 像素/秒。
- **峰值归一。** 只按权重和归一化,每张贴图峰值只有 0.53–0.75——因为偏移的子核不会在任何一点同时贡献。比它替换掉的单渐变暗三分之一,而且各张还不一致。改为按实测最大值重新缩放。
- **毛边。** 对八十五个那么大的核逐像素逐帧扰动 alpha 是 2.6 G 像素/秒。改为一次性烘焙六张 alpha 遮罩,每次重新着色只做一次 `destination-in` 合成。
- **有意重新引入的运动。** search 时缓慢游走,c 上升时向汇聚点靠拢,松手时增大并淡出。每一处都在写之前先对着拖尾比值核过:凝结 0.0017、漂移 0.0004、消散 0.037。
- **双层轨迹。** 头部 34px / alpha 105;残留落下时 44px / 62,1.6 秒寿命结束时 200px / alpha 2。**增长速率和指数衰减分开给**,正在消失的东西才看起来是在扩散变薄,而不只是被调暗。
- **触摸标记。** 先按 spec 删掉圆环、换成三片偏移毛边贴图组成的不规则亮斑,随后按创作者要求把圆环加回来,**放在亮斑外面**(1.55 倍半径),两个记号各司其职:亮斑是材料,说聚了多少;圆环是画面里唯一被画出来的线,说手在哪里。
- **每节点的不透明度。** 之前所有节点共用同一个基准亮度,固定差异只剩宽度,场读起来像同一种物质切成不同大小。现在大小和亮度是两个独立的轴,跨度从 1.3 倍拉到 19 倍。

**决策与原因**

有三样东西必须**烘焙**而不能实时算:合成贴图、边缘遮罩,以及着色本身——后者现在把 c 量化成 32 档,18 秒的按压里重建 31 次而不是 1080 次。这个模式值得点名:**凡是逐像素变化但不逐帧变化的东西,都属于贴图。** 这几条提案能负担得起,全靠认清这一点;若按它们被描述的方式实现,每一条都会因为太贵而被否掉。

消散那条是"必须违背 spec 才能兑现 spec"的例子。它要求场在 1.5 秒内趋近于零。让位置也在这个时间里飞回原处,比值是 0.149,尾巴立刻回来。把快的部分交给半径和不透明度、位置仍按常速回归,读起来是同一回事,比值只有 0.037——因为在任何人能看见位置还在移动之前,alpha 已经没了。
...
- `InkWard_Bound_Interface/INTERFACE_CONTEXT.md` —— §3.0 围绕比值判据重写,新增 §3.3b-2 至 -4
- 比值:凝结 0.0017 · 漂移 0.0004 · 消散 0.037 ·(按飞行方式消散,已否决:0.149)

**反思 / 下一步**

有两个幅度实际上只是它们常量所声称的一小部分,而且都是在追问"为什么看不见的东西看不见"时才被发现的。

第一个是适用于任何分层场的算术。**N 层各自波动 ±A 的东西叠加起来,合计只波动 ±A/√N。** 节点级 ±15% 的闪烁,在二十层深的场里只剩 ±3%——这就是第一次给场加闪烁完全看不出变化的原因。相关的波动不会被这样稀释,所以闪烁被提高了一级:同一团的节点共用一条通道,整个热区一起明灭。回过头看,这也解释了为什么毛边在场的内部几乎看不见、只在最外圈才读得出来——**完全相同的稀释,一天前作为旁注注意到过,却没有和任何东西联系起来,直到它解释了一个 bug。**

第二个更窄但值得写下:**p5 的 `noise()` 叠了四个倍频,几乎从不到达 0 或 1**,实际大多落在 0.3–0.7。因此直接写 `(noise - 0.5) * 2` 只能拿到常量看起来所声称幅度的约四成。文件里所有由 noise 驱动的幅度现在都经过拉伸和钳制,常量才名副其实。

两者属于同一类错误:**表达式是对的,画面上是错的**——不会报错,唯一的症状是某个功能看起来像是没做。凡是有多个因子相乘的地方,幅度应当实测,而不是相信系数。

仍未解决(与"上"相同):这片场只在笔记本上看过,团心位置每次刷新仍会重新随机。

---

## 当前验证清单

仓库能够证明代码和版本历史。完整系统测试已经完成，下列各项均已验证通过：

- [x] Render 显示构建成功并运行服务。
- [x] 浏览器能够打开公网地址。
- [x] 浏览器 HUD 的连接指示灯变绿。
- [x] TouchDesigner WebSocket DAT 连接到 `inkward-bound.onrender.com:443`。
- [x] TouchDesigner 收到 `down`、`move`、`up` 和 `frame` JSON 消息。
- [x] `F` 进入全屏，`F` 或 `Esc` 退出全屏。
- [x] 缓慢稳定的按压与快速躁动的移动产生明显不同的结果——缓慢稳定按压会让轴向视频逐渐推进到最后一帧，快速躁动移动则让播放头在相隔较远的帧之间跳切。

需要说明的是：自 2026 年 7 月 26 日那条记录起，装置改为本地优先运行——Node 中继跑在 Mac 上，TouchDesigner 连 `127.0.0.1:3000`，iPad 走局域网——Render 通道通过 `Touch Server` 开关保留为两次点击即可切换的备用方案。上面几条 Render 相关的勾，记录的是云端通道验证可用，而不是现场实际使用的通道。
